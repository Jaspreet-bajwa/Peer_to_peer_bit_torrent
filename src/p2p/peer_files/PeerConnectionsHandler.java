...
int randomNeighborIndex = -1;
        ...
        for (int i=0; i<thisPeer.getNeighbors().size(); i++) {
            ...
            Future<Object> listenHaveFromNeighborFuture = haveThreadPool.submit(
                    new ListenHaveFromNeighbor(thisPeer.getNeighbors().get(i), logFile, thisPeer, fileManager));

            haveResultsFutures.add(listenHaveFromNeighborFuture);

            randomNeighborIndex = (int) (Math.random() * thisPeer.getNeighbors().size());
        ...
        logFile.optimisticallyUnchokedNeighbor(thisPeer.getNeighbors().get(randomNeighborIndex).getPeerId());
    ...
    public void uploadToNeighbors() throws Exception {
        ...
        while(true) {
            ...
            for (int i=0; i<interestedNeighborsPriorityList.size(); i++) {
                ...
                if(i < config.getCommonConfig().getNoOfPreferredNeighbors() && neighbor.getChockingState().compareAndSet(0, 1)) {
